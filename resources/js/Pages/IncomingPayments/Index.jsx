...
export default function Index({ payments, filters, methods, totalReceived, can }) {
    ...
    const applyFilters = (next = {}) => {
        router.get('/incoming-payments', { search, method: filters.method, ...next }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    return (
        <AppLayout header="Encaissements">
                        ...
                        {payments.data.map((payment) => (
                            <tr key={payment.id} className="text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td className="px-4 py-3">
                                    <Link href={`/incoming-payments/${payment.id}`} className="font-medium text-slate-800 hover:text-orange-600 dark:text-slate-100">
                                        {payment.code}
                                    </Link>
                                    {payment.project && (
                                ...
                                <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                                    {payment.client?.name ?? payment.payer_name ?? '—'}
                                </td>
                                <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                                ...
                                <td className="px-4 py-3 text-right">
                                    <Link href={`/incoming-payments/${payment.id}`} className="text-slate-400 hover:text-orange-600">
