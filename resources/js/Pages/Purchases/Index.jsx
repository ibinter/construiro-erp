...
export default function Index({ orders, filters, statuses, can }) {
    ...
    return (
        <AppLayout header="Bons de commande">
            <Head title={t('Bons de commande')} />
                ...
                >
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder={t('Rechercher un bon de commande…')}
                    ...
                    >
                        ...
                        {statuses.map((s) => (
                            <option key={s} value={s}>{t(PURCHASE_STATUS[s]?.label ?? s)}</option>
                        ))}
                    </select>
                </form>
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            <th className="px-4 py-3">{t('Bon de commande')}</th>
                        ...
                        {orders.data.map((order) => (
                            <tr key={order.id} className="text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td className="px-4 py-3">
                                    ...
                                    {order.project && (
                                        <div className="text-xs text-slate-400">{order.project.name}</div>
                                    )}
                                </td>
                                ...
                                <td className="px-4 py-3 text-right">
                                    <Link href={`/purchases/${order.id}`} className="text-slate-400 hover:text-orange-600">
                        ...
                        {orders.data.length === 0 && (
                            <tr>
                                <td colSpan={6} className="px-4 py-12 text-center text-slate-400">
                                    ...
                                    {t('Aucun bon de commande trouvé.')}
                                </td>
                            </tr>
                        )}
                    </tbody>
