        ...
        <AppLayout header="Plan comptable">
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {accounts.map((account) => {
                            const t = ACCOUNT_TYPE[account.type] ?? { label: account.type, color: 'bg-slate-100 text-slate-600' };
                            return (
            ...
            <Modal show={showModal} onClose={() => setShowModal(false)} maxWidth="md">
                <form onSubmit={submit} className="p-6">
                    <h3 className="mb-4 text-lg font-semibold text-slate-800 dark:text-slate-100">Nouveau compte</h3>
                    <div className="space-y-4">
                        <div>
                            <InputLabel htmlFor="ac_code" value="Code *" />
                        ...
                        <div>
                            <InputLabel htmlFor="ac_label" value="Intitulé *" />
                            <TextInput id="ac_label" className="mt-1 block w-full" placeholder="Achats de marchandises"
                                value={form.data.label} onChange={(e) => form.setData('label', e.target.value)} />
                            <InputError message={form.errors.label} className="mt-1" />
                        </div>
                        <div>
                            <InputLabel htmlFor="ac_type" value="Type *" />
                            <select
                                id="ac_type"
