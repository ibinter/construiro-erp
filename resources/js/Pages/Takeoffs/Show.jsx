    ...
    return (
        <AppLayout header="Fiche métré">
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            ...
                            <th className="px-5 py-3 text-right">Nb</th>
                            <th className="px-5 py-3 text-right">Long.</th>
                            <th className="px-5 py-3 text-right">Larg.</th>
                            <th className="px-5 py-3 text-right">Haut.</th>
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Supprimer ce métré ?</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        Le métré « {takeoff.title} » sera supprimé. Cette action est réversible (corbeille).
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton type="button" onClick={() => setConfirmDelete(false)}>Annuler</SecondaryButton>
                        <DangerButton onClick={deleteTakeoff}>Supprimer définitivement</DangerButton>
                    </div>
                </div>
            </Modal>
        </AppLayout>
