...
export default function Show({ payment, can }) {
    ...
    const deletePayment = () => {
        router.delete(`/outgoing-payments/${payment.id}`);
    };

    const fmtDate = (d) => (d ? new Date(d).toLocaleDateString('fr-FR') : '—');

    return (
        <AppLayout header="Fiche décaissement">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div className="flex items-center gap-2">
                        <Link href="/outgoing-payments" className="text-slate-400 hover:text-orange-600">
                    ...
                    <p className="ml-7 text-sm text-slate-400">
                        {payment.supplier?.name ?? payment.payee_name ?? t('Sans bénéficiaire')}
                ...
                <div className="flex gap-2">
                    {can.update && (
                        <Link
                            href={`/outgoing-payments/${payment.id}/edit`}
            ...
            <div className="mb-6 rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                <div className="text-xs uppercase tracking-wider text-slate-400">{t('Montant décaissé')}</div>
            ...
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                    <div className="flex items-center gap-2 text-slate-400">
                        ...
                        <span className="text-xs uppercase tracking-wider">{t('Bon de commande')}</span>
                    </div>
                    <div className="mt-1 font-semibold text-slate-800 dark:text-slate-100">
                        {payment.purchase_order?.code ?? '—'}
                    </div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                    <div className="flex items-center gap-2 text-slate-400">
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{t('Supprimer ce décaissement ?')}</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        {t('Le décaissement')} « {payment.code} » {t('sera supprimé. Cette action est réversible (corbeille).')}
                    </p>
