    ...
    return (
        <AppLayout header="Fiche matériau">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {belowMin && (
                            <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-700">
                                <span className="h-2 w-2 rounded-full bg-red-500" /> Sous le seuil
                            </span>
                        )}
                    </div>
                    <p className="ml-7 text-sm text-slate-400">
                        {material.code} · {MATERIAL_CATEGORY[material.category] ?? material.category}
                    </p>
                </div>
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
                <InfoTile icon="boxes" label="Stock courant" value={`${Number(material.current_stock).toLocaleString('fr-FR')} ${unit}`} />
                <InfoTile icon="triangle-alert" label="Seuil d'alerte" value={`${Number(material.min_stock).toLocaleString('fr-FR')} ${unit}`} />
                <InfoTile icon="tag" label="Prix de référence" value={formatMoney(material.unit_price)} />
                <InfoTile icon="ruler" label="Unité" value={MATERIAL_UNIT[material.unit] ?? material.unit} />
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
                    ...
                    <Link href="/stocks" className="text-sm text-orange-600 hover:underline">Voir les stocks</Link>
                </div>
                ...
                <ul className="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    {material.movements.length === 0 && (
                        <li className="px-5 py-8 text-center text-sm text-slate-400">
                            Aucun mouvement enregistré pour ce matériau.
                        </li>
                    )}
                </ul>
            </div>
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Supprimer ce matériau ?</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        Le matériau « {material.name} » sera supprimé. Cette action est réversible (corbeille).
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton type="button" onClick={() => setConfirmDelete(false)}>Annuler</SecondaryButton>
                        <DangerButton onClick={deleteMaterial}>Supprimer définitivement</DangerButton>
                    </div>
                </div>
            </Modal>
        </AppLayout>
