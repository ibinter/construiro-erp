...
export default function Show({ unitPrice, can }) {
    ...
    const deleteUnitPrice = () => {
        router.delete(`/unit-prices/${unitPrice.id}`);
    };

    return (
        <AppLayout header="Fiche prix unitaire">
            <Head title={unitPrice.designation} />

            {/* En-tête */}
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div className="flex items-center gap-2">
                        <Link href="/unit-prices" className="text-slate-400 hover:text-orange-600">
                    ...
                    <p className="ml-7 text-sm text-slate-400">{unitPrice.code}</p>
                </div>
