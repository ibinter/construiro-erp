            ...
            <div className="mx-auto max-w-2xl space-y-6">
                <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h2 className="font-semibold text-slate-800 dark:text-slate-100">{t("Fournisseur d'intelligence artificielle")}</h2>
                            <p className="mt-1 text-sm text-slate-500">
                                {t("Aucune clé n'est fournie par le logiciel. Choisissez votre fournisseur et saisissez votre propre clé API. Sans configuration, l'assistant fonctionne avec le moteur de règles interne (gratuit).")}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                    <div className="space-y-5">
                        {/* Fournisseur */}
                        <div>
                            <InputLabel htmlFor="provider" value={t('Fournisseur')} />
                            <select
                                id="provider"
                            ...
                            {current?.docs && (
                                <p className="mt-1 text-xs text-slate-400">
                                    {t('Obtenir une clé :')} <a href={current.docs} target="_blank" rel="noopener" className="text-orange-600 hover:underline">{current.docs}</a>
                                </p>
                            )}
                        </div>
                        ...
                        {!isNone && (
                            <>
                                {/* Clé API */}
                                <div>
                                    <InputLabel htmlFor="api_key" value={t('Clé API')} />
                                    <TextInput
                                        id="api_key"
                                        type="password"
                                        autoComplete="off"
                                        className="mt-1 block w-full"
                                        placeholder={setting.has_key ? t('•••••••• (clé déjà enregistrée — laisser vide pour conserver)') : t('Collez votre clé API')}
                                        value={data.api_key}
                                        onChange={(e) => setData('api_key', e.target.value)}
                                    />
                                    <InputError message={errors.api_key} className="mt-1" />
                                    <p className="mt-1 text-xs text-slate-400">
                                        {t("La clé est chiffrée en base et n'est jamais réaffichée.")}
                                    </p>
                                </div>

                                {/* Modèle */}
                                <div>
                                    <InputLabel htmlFor="model" value={t('Modèle')} />
                                    <TextInput
                                        id="model"
                                        className="mt-1 block w-full"
                                        placeholder={current?.default_model ? `${t('Défaut :')} ${current.default_model}` : ''}
                                        value={data.model}
                                ...
                                {/* URL de base (avancé) */}
                                <div>
                                    <InputLabel htmlFor="base_url" value={t('URL de base (optionnel)')} />
                                    <TextInput
                                        id="base_url"
                                        className="mt-1 block w-full"
                                ...
                                <label className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm text-slate-700 dark:text-slate-200">
                                        {t("Activer l'assistant IA avec ce fournisseur")}
                                    </span>
                                </label>
                            </>
                        )}
                    ...
                    <div className="mt-6 flex items-center justify-between">
                        <Link href="/ai" className="text-sm text-slate-500 hover:underline">{t("← Retour à l'assistant")}</Link>
                        <div className="flex gap-2">
                            {!isNone && (
                                ...
                                >
                                    <Icon name="plug-zap" className="h-4 w-4" /> {testing ? t('Test…') : t('Tester la connexion')}
                                </button>
                            )}
