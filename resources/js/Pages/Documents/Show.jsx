    ...
    return (
        <AppLayout header="Fiche document">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="inline-flex rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-medium text-orange-700">
                            {t(CATEGORY[document.category] ?? document.category)}
                        </span>
                    </div>
                ...
                <div className="flex gap-2">
                    {(document.file_url || document.file_name) && (
                        ...
                        >
                            <Icon name="download" className="h-4 w-4" /> {t('Télécharger')}
                        </a>
                    )}
                    {can.update && (
                        <Link
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
                <InfoTile icon="file" label={t('Fichier')} value={document.file_name || '—'} />
                <InfoTile icon="hard-drive" label={t('Taille')} value={document.size_kb != null ? `${document.size_kb} Ko` : '—'} />
                <InfoTile icon="folder-kanban" label={t('Projet')} value={document.project?.name || '—'} />
                <InfoTile icon="user" label={t('Déposé par')} value={document.uploaded_by || '—'} />
            </div>
            ...
            <div className="mb-6 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                <h3 className="mb-3 flex items-center gap-2 font-semibold text-slate-800 dark:text-slate-100">
                    <Icon name="info" className="h-5 w-5 text-orange-500" /> {t('Métadonnées du fichier')}
                </h3>
                <dl className="space-y-2 text-sm">
                    <div className="flex justify-between gap-4">
                        <dt className="text-slate-400">{t('Chemin / URL')}</dt>
                        <dd className="break-all text-right text-slate-700 dark:text-slate-200">{document.file_path || '—'}</dd>
                    </div>
                    <div className="flex justify-between">
                        <dt className="text-slate-400">{t('Type MIME')}</dt>
                        <dd className="text-slate-700 dark:text-slate-200">{document.mime_type || '—'}</dd>
                    </div>
                    <div className="flex justify-between">
                        <dt className="text-slate-400">{t('Version')}</dt>
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{t('Supprimer ce document ?')}</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        {t('Le document')} « {document.title} » {t('sera supprimé. Cette action est réversible (corbeille).')}
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton type="button" onClick={() => setConfirmDelete(false)}>{t('Annuler')}</SecondaryButton>
                        <DangerButton onClick={deleteDocument}>{t('Supprimer définitivement')}</DangerButton>
                    </div>
                </div>
            </Modal>
        </AppLayout>
