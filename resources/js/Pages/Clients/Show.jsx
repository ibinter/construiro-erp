    ...
    return (
        <AppLayout header="Fiche client">
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
                <InfoTile icon="user" label="Contact" value={client.contact_name || '—'} />
                <InfoTile icon="phone" label="Téléphone" value={client.phone || '—'} />
                <InfoTile icon="mail" label="E-mail" value={client.email || '—'} />
                <InfoTile icon="map-pin" label="Ville" value={client.city || '—'} />
            </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <dl className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <dd className="text-slate-700 dark:text-slate-200">{CLIENT_TYPE[client.type] ?? client.type}</dd>
                        </div>
                        <div className="flex justify-between">
                            <dt className="text-slate-400">Statut</dt>
                            <dd className="text-slate-700 dark:text-slate-200">{client.is_active ? 'Actif' : 'Inactif'}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Supprimer ce client ?</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        Le client « {client.name} » sera supprimé. Cette action est réversible (corbeille).
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton type="button" onClick={() => setConfirmDelete(false)}>Annuler</SecondaryButton>
                        <DangerButton onClick={deleteClient}>Supprimer définitivement</DangerButton>
                    </div>
                </div>
            </Modal>
        </AppLayout>
