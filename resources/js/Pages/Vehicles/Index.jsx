...
// Vue filtrée du parc matériel : véhicules uniquement (category = 'vehicule').
...
export default function Index({ vehicles, filters, statuses, can }) {
    const { t } = useTrans();
    const [search, setSearch] = useState(filters.search ?? '');

    const applyFilters = (next = {}) => {
        router.get('/vehicles', { search, status: filters.status, ...next }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    return (
        <AppLayout header="Véhicules">
            <Head title={t('Véhicules')} />
                        ...
                        {vehicles.data.map((item) => (
                            <tr key={item.id} className="text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                <td className="px-4 py-3">
                                    <Link href={`/vehicles/${item.id}`} className="font-medium text-slate-800 hover:text-orange-600 dark:text-slate-100">
                                        {item.name}
                                    </Link>
                                ...
                                <td className="px-4 py-3 text-right">
                                    <Link href={`/vehicles/${item.id}`} className="text-slate-400 hover:text-orange-600">
                        ...
                        {vehicles.data.length === 0 && (
                            <tr>
                                <td colSpan={7} className="px-4 py-12 text-center text-slate-400">
                                    <Icon name="car" className="mx-auto mb-2 h-8 w-8" />
