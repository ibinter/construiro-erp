    ...
    return (
        <AppLayout header="Fiche étude">
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
                <InfoTile icon="ruler" label={t('Type')} value={t(STUDY_TYPE[study.type] ?? study.type)} />
                <InfoTile icon="user" label={t('Auteur')} value={study.author || '—'} />
                <InfoTile icon="folder-kanban" label={t('Projet')} value={study.project ? study.project.name : '—'} />
                <InfoTile icon="check-circle" label={t('Statut')} value={t(s.label)} />
            </div>
            ...
            <Modal show={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="md">
                <div className="p-6">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">{t('Supprimer cette étude ?')}</h3>
                    <p className="mt-2 text-sm text-slate-500">
                        {t("L'étude")} « {study.title} » {t('sera supprimée. Cette action est réversible (corbeille).')}
                    </p>
                    <div className="mt-6 flex justify-end gap-3">
                        <SecondaryButton type="button" onClick={() => setConfirmDelete(false)}>{t('Annuler')}</SecondaryButton>
                        <DangerButton onClick={deleteStudy}>{t('Supprimer définitivement')}</DangerButton>
                    </div>
                </div>
            </Modal>
        </AppLayout>
