                ...
                >
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder={t('Rechercher une étude…')}
                    ...
                    >
                        ...
                        {statuses.map((s) => (
                            <option key={s} value={s}>{t(STUDY_STATUS[s]?.label ?? s)}</option>
                        ))}
                    </select>
                </form>
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-800/50">
                        <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                            <th className="px-4 py-3">{t('Étude')}</th>
                            <th className="px-4 py-3">{t('Type')}</th>
                            <th className="px-4 py-3">{t('Auteur')}</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {studies.data.map((study) => (
                            <tr key={study.id} className="text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{t(STUDY_TYPE[study.type] ?? study.type)}</td>
