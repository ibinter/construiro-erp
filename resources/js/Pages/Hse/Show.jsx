...
export default function Show({ incident, can }) {
    ...
    const deleteIncident = () => {
        router.delete(`/hse/${incident.id}`);
    };

    const fmtDate = (d) => (d ? new Date(d).toLocaleDateString('fr-FR') : '—');

    return (
        <AppLayout header="Fiche incident QHSE">
            ...
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <div className="flex flex-wrap items-center gap-2">
                        <Link href="/hse" className="text-slate-400 hover:text-orange-600">
                            <Icon name="arrow-left" className="h-5 w-5" />
                        </Link>
                        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{incident.title}</h2>
                        <Badge map={INCIDENT_SEVERITY} value={incident.severity} />
