...
export default function Account({ account, transactions }) {
    const { t } = useTrans();
    const ty = ACCOUNT_TYPE[account.type] ?? { label: account.type, color: 'bg-slate-100 text-slate-600', icon: 'wallet' };
        ...
        <AppLayout header="Compte de trésorerie">
            ...
            <div className="mb-6 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                <div className="flex items-center gap-2">
                    ...
                    <Icon name={ty.icon} className="h-6 w-6 text-slate-400" />
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div>
                        <div className="text-xs uppercase tracking-wider text-slate-400">{t('Solde courant')}</div>
                        <div className="mt-1 text-2xl font-bold text-orange-600">{formatMoney(account.balance, account.currency)}</div>
                    </div>
                    <div>
                        <div className="text-xs uppercase tracking-wider text-slate-400">{t("Solde d'ouverture")}</div>
                    ...
                    <div>
                        <div className="text-xs uppercase tracking-wider text-slate-400">{t('Banque / Opérateur')}</div>
                            ...
                            return (
                                <tr key={tx.id} className="text-sm hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${tt.color}`}>{t(tt.label)}</span>
                                    </td>
                        ...
                        {transactions.data.length === 0 && (
                            <tr>
                                <td colSpan={7} className="px-4 py-12 text-center text-slate-400">
                                    ...
                                    {t('Aucune transaction sur ce compte.')}
                                </td>
                            </tr>
                        )}
                    </tbody>
