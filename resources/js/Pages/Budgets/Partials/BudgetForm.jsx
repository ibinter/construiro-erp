        ...
        <form onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {field('code', t('Code budget *'), { placeholder: 'BUD-2026-001' })}
                    {field('title', t('Intitulé *'), { placeholder: t('Budget chantier R+8 Plateau') })}
                    {field('fiscal_year', t('Exercice *'), { type: 'number', min: 2000, max: 2100 })}
                    ...
                    <div>
                        ...
                        >
                            {(statuses.length ? statuses : Object.keys(BUDGET_STATUS)).map((s) => (
                                <option key={s} value={s}>{t(BUDGET_STATUS[s] ?? s)}</option>
                            ))}
                        </select>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="font-semibold text-slate-800 dark:text-slate-100">{t('Postes budgétaires')}</h3>
                    <button
                        type="button"
                        onClick={addLine}
                    ...
                    >
                        <Icon name="plus" className="h-4 w-4" /> {t('Ajouter un poste')}
                    </button>
                </div>
                    ...
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                                <th className="pb-2 pr-2 w-40">{t('Catégorie')}</th>
                                <th className="pb-2 px-2">{t('Libellé')}</th>
                                <th className="pb-2 px-2 w-40">{t('Planifié')}</th>
                                <th className="pb-2 px-2 w-40">{t('Réalisé')}</th>
                                ...
                                <tr key={i} className="align-top">
                                    ...
                                    <td className="py-1 px-2">
                                        <TextInput
                                            ...
                                            placeholder={t('Approvisionnement béton')}
                                        />
                ...
                <div className="mt-6 flex justify-end">
                    <div className="w-full max-w-xs space-y-2 text-sm">
                        <div className="flex justify-between text-slate-600 dark:text-slate-300">
                            <span>{t('Total planifié')}</span>
                            <span>{formatMoney(totalPlanned, data.currency)}</span>
                        </div>
                        <div className="flex justify-between text-slate-600 dark:text-slate-300">
                            <span>{t('Total réalisé')}</span>
