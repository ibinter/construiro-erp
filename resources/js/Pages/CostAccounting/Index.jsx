        ...
        <AppLayout header="Comptabilité analytique">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="rounded-xl border border-orange-200 bg-orange-50 p-5 dark:border-orange-900/50 dark:bg-orange-950/30">
                    ...
                    <div className="mt-1 flex justify-between text-xs text-orange-600/70 dark:text-orange-400/70">
                        <span>Produits : {formatMoney(totals.produits)}</span>
                        <span>Charges : {formatMoney(totals.charges)}</span>
                    </div>
                </div>
                    ...
                    return (
                        <div key={axisKey} className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-slate-800 dark:text-slate-100">{a.label}</span>
                            </div>
                            ...
                            <div className="mt-1 flex justify-between text-xs text-slate-400">
                                <span>Prod. {formatMoney(row.produits)}</span>
                                <span>Chg. {formatMoney(row.charges)}</span>
                            </div>
                        </div>
                    );
                })}
            ...
            <div className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
                    ...
                    <h3 className="font-semibold text-slate-800 dark:text-slate-100">Écritures analytiques</h3>
                </div>
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {entries.data.map((entry) => {
                            const t = ENTRY_TYPE[entry.type] ?? { label: entry.type, color: 'bg-slate-100 text-slate-600' };
            ...
            <Modal show={showModal} onClose={() => setShowModal(false)}>
                <form onSubmit={submit} className="p-6">
                    <h3 className="mb-4 text-lg font-semibold text-slate-800 dark:text-slate-100">Nouvelle écriture analytique</h3>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                            <InputLabel htmlFor="ca_label" value="Libellé *" />
                            <TextInput id="ca_label" className="mt-1 block w-full" placeholder="Achat ciment CPA 45"
                                value={form.data.label} onChange={(e) => form.setData('label', e.target.value)} />
                            <InputError message={form.errors.label} className="mt-1" />
                        </div>
                        <div>
                            <InputLabel htmlFor="ca_axis" value="Axe analytique *" />
                            <select
                                id="ca_axis"
                            ...
                            >
                                {axes.map((a) => (
                                    <option key={a} value={a}>{AXIS[a]?.label ?? a}</option>
                                ))}
                            </select>
                            <InputError message={form.errors.axis} className="mt-1" />
                        </div>
                        <div>
                            <InputLabel htmlFor="ca_type" value="Type *" />
                            <select
                                id="ca_type"
                                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"
                            ...
                            >
                                {types.map((t) => (
                                    <option key={t} value={t}>{ENTRY_TYPE[t]?.label ?? t}</option>
                                ))}
                            </select>
                            <InputError message={form.errors.type} className="mt-1" />
                        </div>
                        <div>
                            <InputLabel htmlFor="ca_amount" value="Montant *" />
                        ...
                        <div>
                            <InputLabel htmlFor="ca_date" value="Date *" />
                        ...
                        <div>
                            <InputLabel htmlFor="ca_project" value="Projet" />
                            <select
                                id="ca_project"
                                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"
                        ...
                        <div>
                            <InputLabel htmlFor="ca_reference" value="Référence" />
