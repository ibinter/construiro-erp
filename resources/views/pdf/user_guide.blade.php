...
<div class="cover">
    ...
    <div class="cover-badge">ERP BTP â€¢ IBIG Soft</div>
    <div class="cover-title">{{ $locale === 'fr' ? 'Guide Utilisateur' : 'User Guide' }}</div>
    <div class="cover-sub">{{ $locale === 'fr' ? 'L\'ERP BTP conÃ§u pour l\'Afrique' : 'The BTP ERP designed for Africa' }}</div>
    <div class="cover-meta">
        {{ $locale === 'fr' ? 'Version' : 'Version' }} {{ $version }} â€” {{ now()->format('Y') }}<br>
        {{ $locale === 'fr' ? 'IBIG SARL â€” Intermark Business International Group' : 'IBIG SARL â€” Intermark Business International Group' }}
    </div>
</div>

{{-- TABLE DES MATIÃˆRES --}}
<div class="page">
    <h1>{{ $locale === 'fr' ? 'Table des matiÃ¨res' : 'Table of Contents' }}</h1>
    <ul class="toc">
        <li><span>{{ $locale === 'fr' ? '1. Introduction Ã  CONSTRUIRO' : '1. Introduction to CONSTRUIRO' }}</span><span>2</span></li>
        ...
        <li><span>{{ $locale === 'fr' ? '7. Stock & MatÃ©riaux' : '7. Stock & Materials' }}</span><span>8</span></li>
        <li><span>{{ $locale === 'fr' ? '8. Ã‰quipements & Engins' : '8. Equipment & Machinery' }}</span><span>9</span></li>
        <li><span>{{ $locale === 'fr' ? '9. Devis & Facturation' : '9. Quotes & Invoicing' }}</span><span>10</span></li>
        <li><span>{{ $locale === 'fr' ? '10. Finance & ComptabilitÃ©' : '10. Finance & Accounting' }}</span><span>11</span></li>
        <li><span>{{ $locale === 'fr' ? '11. HSE & QualitÃ©' : '11. HSE & Quality' }}</span><span>12</span></li>
        <li><span>{{ $locale === 'fr' ? '12. Bureau d\'Ã©tudes' : '12. Design Office' }}</span><span>13</span></li>
        <li><span>{{ $locale === 'fr' ? '13. ParamÃ¨tres & Administration' : '13. Settings & Administration' }}</span><span>14</span></li>
...
{{-- CHAPITRE 1 --}}
<div class="page">
    <h1>{{ $locale === 'fr' ? '1. Introduction Ã  CONSTRUIRO' : '1. Introduction to CONSTRUIRO' }}</h1>
    @if($locale === 'fr')
    <p>CONSTRUIRO est un ERP (Enterprise Resource Planning) spÃ©cialement conÃ§u pour les entreprises du BTP et de la construction en Afrique. Il centralise en une seule plateforme tous les processus mÃ©tier : projets, chantiers, ressources humaines, stock, Ã©quipements, finance et comptabilitÃ©.</p>
    <h2>Pourquoi CONSTRUIRO ?</h2>
    <ul>
        <li><strong>15+ modules mÃ©tier</strong> intÃ©grÃ©s et interconnectÃ©s</li>
        <li><strong>Multi-entreprise</strong> : isolez les donnÃ©es de chaque filiale</li>
        <li><strong>Multi-devise</strong> : XOF, XAF, GNF, USD, EUR et plus</li>
        <li><strong>Multi-langue</strong> : FranÃ§ais et Anglais</li>
        <li><strong>Accessible partout</strong> : navigateur web sur PC, tablette et mobile</li>
    </ul>
    <h2>Architecture de la plateforme</h2>
    <p>CONSTRUIRO est organisÃ© en <strong>modules</strong>, accessibles depuis la barre latÃ©rale gauche. Chaque utilisateur n'accÃ¨de qu'aux modules autorisÃ©s par son rÃ´le.</p>
    <div class="tip">
        <div class="tip-title">ðŸ’¡ Conseil</div>
        Votre administrateur peut personnaliser les rÃ´les et permissions depuis Administration â†’ Utilisateurs.
    </div>
    @else
    ...
    <div class="tip">
        ...
        Your administrator can customize roles and permissions from Administration â†’ Users.
    </div>
    @endif
</div>
...
<div class="page">
    ...
    <div class="tip">
        <div class="tip-title">ðŸ” Mot de passe oubliÃ©</div>
        Cliquez sur "Mot de passe oubliÃ©" sur la page de connexion. Un lien de rÃ©initialisation sera envoyÃ© Ã  votre email.
    </div>
    <h2>Navigation principale</h2>
    <ul>
        <li><strong>Barre latÃ©rale gauche</strong> : accÃ¨s Ã  tous les modules</li>
        <li><strong>Barre supÃ©rieure</strong> : notifications, paramÃ¨tres de compte, dÃ©connexion</li>
        <li><strong>Fil d'Ariane</strong> : navigation entre les niveaux (ex: Projets â†’ Projet A â†’ Chantiers)</li>
    </ul>
    @else
    <h2>Logging in</h2>
    <ol>
    ...
    <div class="tip">
        <div class="tip-title">ðŸ” Forgot password</div>
    ...
    <ul>
        ...
        <li><strong>Breadcrumb</strong>: navigation between levels (e.g.: Projects â†’ Project A â†’ Sites)</li>
    </ul>
    @endif
</div>

{{-- CHAPITRES 3-14 condensÃ©s --}}
@php
$chapters = $locale === 'fr' ? [
    ['3. Tableau de bord', 'Le tableau de bord affiche une synthÃ¨se en temps rÃ©el de votre activitÃ© : KPIs financiers, projets en cours, alertes, graphiques. Chaque carte est cliquable pour accÃ©der au module correspondant.'],
    ['4. Gestion des projets', 'CrÃ©ez et suivez vos projets BTP avec budget, planning, avancement et Ã©quipe. Chaque projet peut contenir plusieurs chantiers. Utilisez le Gantt intÃ©grÃ© pour visualiser le planning.'],
    ['5. Gestion des chantiers', 'Chaque chantier est rattachÃ© Ã  un projet. GÃ©rez les tÃ¢ches, pointages, incidents QHSE et demandes d\'approvisionnement directement depuis la fiche chantier.'],
    ['6. RH & Paie', 'Enregistrez vos employÃ©s, suivez leurs prÃ©sences, gÃ©rez les congÃ©s et gÃ©nÃ©rez les fiches de paie. Supporte les cotisations CNPS et les barÃ¨mes locaux.'],
    ['7. Stock & MatÃ©riaux', 'GÃ©rez votre catalogue de matÃ©riaux, vos entrepÃ´ts et vos mouvements de stock. Les bons de commande fournisseurs alimentent automatiquement le stock Ã  la rÃ©ception.'],
    ['8. Ã‰quipements & Engins', 'Enregistrez votre parc roulant et vos engins. Planifiez les maintenances prÃ©ventives, suivez les consommations de carburant et affectez les Ã©quipements aux chantiers.'],
    ['9. Devis & Facturation', 'CrÃ©ez des devis Ã  partir du bordereau de prix unitaires, transmettez-les aux clients et gÃ©nÃ©rez les factures en un clic. Export PDF disponible.'],
    ['10. Finance & ComptabilitÃ©', 'GÃ©rez votre trÃ©sorerie, votre plan comptable SYSCOHADA, le journal des Ã©critures, les budgets et les Ã©tats financiers. Exports Excel et PDF.'],
    ['11. HSE & QualitÃ©', 'Enregistrez les incidents de sÃ©curitÃ©, les contrÃ´les qualitÃ© et les non-conformitÃ©s. GÃ©nÃ©rez les rapports HSE et suivez les actions correctives.'],
    ['12. Bureau d\'Ã©tudes', 'GÃ©rez votre bibliothÃ¨que de prix unitaires, vos mÃ©trÃ©s, DQE et Ã©tudes techniques. Liez les Ã©tudes aux projets pour un suivi intÃ©grÃ©.'],
    ['13. ParamÃ¨tres & Administration', 'Administrez les utilisateurs et rÃ´les, paramÃ©trez la devise et la langue, personnalisez les modules actifs et gÃ©rez les pages lÃ©gales.'],
    ['14. Support & Aide', 'AccÃ©dez au centre de support depuis le menu principal. CrÃ©ez un ticket, suivez son avancement et consultez la base de connaissances pour les rÃ©ponses rapides.'],
] : [
