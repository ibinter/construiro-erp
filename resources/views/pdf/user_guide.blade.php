...
$chapters = $locale === 'fr' ? [
    ...
    ['15. Licence, essai et palier Découverte', "Votre espace connaît 6 états. Demo publique : le vrai logiciel avec des données fictives, sans inscription. Découverte : palier gratuit à vie, plafonné à {$lic['quota']} chantier, sans carte bancaire (exclut l'export, le multi-utilisateur, l'API et l'assistant IA). Essai : {$lic['essai']} jours sur la formule Pro complète, sans carte bancaire ; à la fin, votre espace bascule automatiquement en Découverte, aucune donnée n'est supprimée (l'usage au-delà du plafond passe en lecture seule). Abonnement actif : toutes fonctions ouvertes. Période de grâce : {$lic['grace']} jours après l'échéance, accès complet maintenu. Lecture seule : après la grâce, vos données restent consultables et sont conservées {$lic['retention']} jours avant purge (2 avertissements par e-mail). Aucune licence n'est perpétuelle. Filigrane : les documents générés au palier Découverte portent la mention « {$lic['filigrane']} », retirée dès le premier paiement. Une prolongation d'essai de 15 jours peut être accordée une seule fois sur demande."],
] : [
    ...
    ['15. License, trial and Discover plan', "Your workspace has 6 states. Public demo: the real software with fictitious data, no sign-up. Discover: free forever plan, capped at {$lic['quota']} project site, no credit card (excludes export, multi-user, API and the AI assistant). Trial: {$lic['essai']} days on the full Pro plan, no credit card; at the end, your workspace automatically switches to Discover with no data deleted (usage beyond the cap becomes read-only). Active subscription: all features open. Grace period: {$lic['grace']} days after expiry, full access maintained. Read-only: after grace, your data stays viewable and is kept {$lic['retention']} days before purge (2 email warnings). No perpetual license. Watermark: documents generated on the Discover plan carry the notice, removed on first payment. A one-time 15-day trial extension may be granted on request."],
];
@endphp
